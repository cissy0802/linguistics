...
**若 N_pub ≥ N_top（TOPICS 已全部写完）**：本次**不生成任何页面、不修改 TOPICS.md、绝不自己发明新主题**；直接结束（至多发一条『🔤 linguistics 已写完 TOPICS 路线图，补充新主题后自动续写』的通知，且仅在你判断这是刚写完的当次时发，不要每次都发）。**这条优先级高于本仓 routine prompt 里任何"超表就自造新主题"的旧措辞。** 新主题只由 BigCat 手动或 deep-research 反哺加进 TOPICS.md；加了之后守卫会自动重开、routine 自然续写。
...
命中就修掉（删中文节点或译成英文）再 publish。

**⚠️ 本仓的白名单比别的仓宽——语言例子是内容本身，不是泄漏。** 英文页里下列汉字（及日文、韩文、阿拉伯文等任何非拉丁文字）**必须保留，删掉就是把内容删了**：
- 作为**语言学例子**出现的字词句：声调对比（mā / má / mǎ / mà 配「妈 麻 马 骂」）、汉字结构、量词、分词演示（「研究/生命/起源」vs「研究生/命/起源」）、跨语言对比表里的原文列、borrowing 的原词。
- 例子**必须挂英文注解**：汉字后面要有拼音或罗马转写 + 英文释义（`马 (mǎ, "horse")`），孤零零一个汉字站在英文句子里没有解释 = 按泄漏处理。
- 语言切换标签『中文』、term(中文) 括注。

**仍然算泄漏、必须修掉**的是：正文叙述句、小节标题、图注、SVG 标签、`.en-brief` 残留、按钮与导航文案里出现的中文——也就是**任何"本该是英文行文"的位置**。判据一句话：**这个汉字是被谈论的对象（保留），还是在拿它说话（泄漏）？**

`tools/check-page.py` **已经实现了这条判据**（2026-08-30）：它只报「汉字未挂注解」的段落，带括号转写或引号释义的例子自动放行。所以它报出来的就是真泄漏，直接修；不要再当误报跳过。
